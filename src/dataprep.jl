function prepare_data_turing(data, data_r, data_label_p, tau; population = ["ASDC", "cDC1", "DC2"], individual = ["C66", "C67", "C68", "C52", "C53", "C55"], ratios = ["RASDC", "RcDC1", "RDC2", "RASDC_cDC1_bm", "RASDC_DC2_bm", "RASDC_cDC1_blood", "RASDC_DC2_blood"], population_mapping =[j => idx for (idx, j) in enumerate(population)], label_p_names = [:fr,:delta, :frac], ratio_approach="2", ratio_summary = "mean", mean_data::Bool = false)
