...
begin
	...
	priors = (;p_DC3bm = (@pipe df_p_priors |> subset(_, :parameter => (x -> x .== "DC3")) |> _[1,:] |> create_dist(_.dist, _.μ, _.σ, 1, 8e-12, _.upper)))
end
...
begin
	if !isfile(projectdir("notebooks","02_fitting","01_lognormal_prior",notebook_folder,"df_ppc.csv"))
		ppc = @pipe sample_mcmc(chains, 200) |> get_posterior_predictive(turing_model_ppc, _) |> [_[j] for j in 1:length(_)]
