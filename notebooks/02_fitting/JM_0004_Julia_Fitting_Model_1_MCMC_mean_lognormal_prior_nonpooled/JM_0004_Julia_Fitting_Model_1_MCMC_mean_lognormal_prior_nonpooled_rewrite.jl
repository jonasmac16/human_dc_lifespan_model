...
load(projectdir("model_figures","model_"*model_id*".pdf"))
...
begin
	df_p_priors = DataFrame(load(datadir("exp_pro", "p_priors_truncatednormal.csv")))

	priors = (p_ASDCbm = truncated(Normal((@linq df_p_priors |> where(:parameter .== "ASDC") |> DataFrames.select([:µ, :σ]) |> Array |> reshape(:))...), 0.0, Inf),
							p_cDC1bm = truncated(Normal((@linq df_p_priors |> where(:parameter .== "cDC1") |> DataFrames.select([:µ, :σ]) |> Array |> reshape(:))...), 0.0, Inf),
							p_DC2bm = truncated(Normal((@linq df_p_priors |> where(:parameter .== "DC2") |> DataFrames.select([:µ, :σ]) |> Array |> reshape(:))...), 0.0, Inf))
end
...
begin
	...
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"mcmc_res.jlso")))
		if isnothing(parallel_sampling_method)
			chains = mapreduce(c -> sample(turing_model, NUTS(10, accept_rate), 20, progress=false), chainscat, 1:n_chains)
		else
...
p_ppc = plot_ppc(ppc_b, data_in, 1:3, subplotkwargs=(; alpha=0.1);title= permutedims([((permutedims(donor_ids) .* " ") .* ["ASDC", "cDC1","DC2"])...]), size=(1000,1000), legend=false)
else
...
begin
	...
	p_ppc_bm = plot_ppc(ppc_bm, 1:3, subplotkwargs=(; alpha=0.1);title= permutedims([((permutedims(donor_ids) .* " ") .* ["ASDC (bm)", "cDC1 (bm)","DC2 (bm)"])...]), size=(1000,1000), legend=false)
	else
