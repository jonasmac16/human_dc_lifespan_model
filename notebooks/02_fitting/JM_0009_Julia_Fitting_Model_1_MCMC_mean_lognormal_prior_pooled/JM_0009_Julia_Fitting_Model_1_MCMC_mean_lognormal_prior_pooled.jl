...
# ╔═╡ a2b56a4b-b6ab-4ded-9d61-fb26b30db730
using DrWatson
...
include(projectdir("models", "turing", "revised_models", "mean", "pooled", "turing_predc_cdc1_cdc2_model_"*model_id*".jl"))
...
solver_in = AutoVern9(KenCarp4(),lazy=false)
...
function plot_ppc(sols, vars;cols=[:grey, :grey, :grey], subplotkwargs=(;), kwargs...)
	...
	for j in sols
		for (idx, k) in enumerate(j)
			for l in vars
				plot!(p, k, vars=l,colour= cols[l], subplot=(idx-1)*3 + l; subplotkwargs...) 
			end
		end
	end
	return p
...
function plot_ppc(sols, data, vars; subplotkwargs=(;), kwargs...)
	p = plot_ppc(sols, vars, subplotkwargs=subplotkwargs; kwargs...)
	col =  ["red", "blue", "green"]
	
	for j in 1:length(data.data)
		scatter!(p, [data.metadata.timepoints[data.metadata.order.donor[j]][data.metadata.order.timepoint_idx[j]]], [data.data[j]], subplot=(data.metadata.order.donor[j]-1)*3 + data.metadata.order.population[j], c = col[data.metadata.order.population[j]])
	end
	return p
end
...
begin
	turing_model_ppc_bm = _turing_model(data_in.data, data_in.metadata, mtk_problem, solver_in, priors, ode_parallel_mode=solver_parallel_methods; ode_args=(abstol=1e-10, reltol=1e-10, maxiters=1e8, save_idxs=[1,2,3]))
end	
...
begin
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit.pdf")))
		...
		p_ppc = plot_ppc(ppc_b, data_in, 1:3, subplotkwargs=(; alpha=0.1);title= permutedims([((permutedims(donor_ids) .* " ") .* ["preDC", "cDC1","cDC2"])...]), size=(1000,1000), legend=false)
	else
...
begin
	...
	p_ppc_bm = plot_ppc(ppc_bm, 1:3, subplotkwargs=(; alpha=0.1);title= permutedims([((permutedims(donor_ids) .* " ") .* ["preDC (bm)", "cDC1 (bm)","cDC2 (bm)"])...]), size=(1000,1000), legend=false)
	else
...
begin
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"df_mcmc_comp.jlso")))
		df_par_all = DataFrame(p_preDCbm=Float64[], δ_preDCbm=Float64[], p_cDC1bm=Float64[], δ_cDC1bm=Float64[], p_cDC2bm=Float64[], δ_cDC2bm=Float64[], δ_preDCb=Float64[], δ_cDC1b=Float64[], δ_cDC2b=Float64[], λ_preDC=Float64[], λ_cDC1=Float64[], λ_cDC2=Float64[], Δ_cDC1bm=Float64[], Δ_cDC2bm=Float64[], Δ_cDC1b=Float64[], Δ_cDC2b=Float64[])
...
# ╔═╡ 795dab21-1489-4dc5-9f3f-41f62d79ab2e
...
# ╔═╡ f2603f71-2788-4ebf-bad1-ec5b90bb602c
DrWatson.@quickactivate "Model of DC Differentiation"
...
git-tree-sha1 = "af78f2e1d13f9bc29780d567a6450b53bae3270f"
