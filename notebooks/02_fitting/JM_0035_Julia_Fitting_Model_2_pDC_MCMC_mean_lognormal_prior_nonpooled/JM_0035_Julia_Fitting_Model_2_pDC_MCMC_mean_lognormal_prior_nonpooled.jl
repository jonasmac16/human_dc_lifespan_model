...
include(projectdir("models", "turing", "revised_models", "mean", "nonpooled", "turing_pdc_model_"*model_id*".jl"))
...
begin
	...
	model(du,u,h,p,t) = eval(Symbol("_model_pdc_"*model_id))(du,u,h,p,t, U_func, data_in.metadata.R)
...
begin
	...
	df_wide = vcat([(@pipe vec_sol[k].u |> _[j] |> DataFrame(_) |> rename(_, "value1" => "pDC_bm", "value2" => "pDC_b") |> insertcols!(_, :donor => donor_ids[j], :sample_idx=>k)) for k in 1:length(vec_sol) for j in 1:length(vec_sol[k])]...)
