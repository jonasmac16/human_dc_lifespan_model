...
# ╔═╡ 6b7d3ef3-9417-47c5-8082-486c3a6d6e23
using DrWatson

# ╔═╡ 128ec809-662f-4ee6-98ed-194fa0cecc3a
...
begin
	...
	mkpath(projectdir("notebooks","02_fitting",notebook_folder))
...
p_init = ones([8,9,8][tryparse(Int, model_id)])
...
begin
	...
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"mcmc_res.jlso")))
		...
		JLSO.save(projectdir("notebooks", "02_fitting", notebook_folder,"mcmc_res.jlso"), :chain => chains)
	else
		chains = JLSO.load(projectdir("notebooks", "02_fitting", notebook_folder,"mcmc_res.jlso"))[:chain]
	end
end
...
begin
	io = open(projectdir("notebooks", "02_fitting", notebook_folder,"check_diagnostics.txt"), "w+")
	...
	io_read = open(projectdir("notebooks", "02_fitting", notebook_folder,"check_diagnostics.txt"), "r")
		...
		for j in sols
			for (idx, k) in enumerate(j)
				for (idx2, l) in enumerate(vars)
					plot!(p, k, vars=l,colour= cols[idx2], subplot=(idx-1) + idx2; subplotkwargs...) 
				end
			end
		end
		return p
...
begin
	...
	function plot_ppc(sols, data, vars; cols=[colorant"#755494", colorant"#de3458", colorant"#4e65a3"],  subplotkwargs=(;), kwargs...)
		...
		for j in 1:length(data.data)
			scatter!(p, [data.metadata.timepoints[data.metadata.order.donor[j]][data.metadata.order.timepoint_idx[j]]], [data.data[j]], subplot=(data.metadata.order.donor[j]-1) + data.metadata.order.population[j], c = cols[data.metadata.order.population[j]], yerror=data.data_sd[j])
		end
		return p
	end
...
begin
	if !isfile(projectdir("notebooks", "02_fitting", notebook_folder,"df_ppc.csv"))
		ppc = @pipe sample_mcmc(chains, 50) |> 
		# force_high_prol_rate(_, 2.5) |>
		get_posterior_predictive(turing_model_ppc, _) |> [_[j] for j in 1:length(_)]
		df_ppc = create_model_prediction_df(ppc)
		save(projectdir("notebooks", "02_fitting", notebook_folder,"df_ppc.csv"), df_ppc)
	end
end

# ╔═╡ 7d95653b-2ee4-4e1f-82cf-3e80a7e544ab
begin
	test_par_df = sample_mcmc(chains, 2)
	# transform!(test_par_df, Symbol("par[1][1]") => (x -> repeat([1.5], length(x))), renamecols=false)
	test_par_df
end

# ╔═╡ fc91ec7a-6b14-4c60-956c-0d645276ebca
function force_high_prol_rate(chains_obj, prate)
	chains_obj.value[:,1,:] .= prate
	return chains_obj
end

# ╔═╡ 6361e4f1-5251-4c7f-a5ea-766c5870d5cb
test_par_df.value[:,1,:] .= 2.5

# ╔═╡ 10de4e0d-8428-4b6d-9afb-af1802ac6b88


# ╔═╡ d99a1b37-55be-44b5-a93a-68c84c0678a9
test_par_df

# ╔═╡ 48482cbf-be30-4b44-8e55-2fa231f742e9


# ╔═╡ f8200d8a-d7c1-4bae-9e71-efb41c3be8d5


# ╔═╡ a24c7d72-9f01-49d9-9e7d-883ec0d6835a


# ╔═╡ 8269bc54-c84f-417d-9b94-84723d305285


# ╔═╡ d0bbb385-2ba6-43dd-8e41-1d6780f01928


# ╔═╡ 051bd707-6187-462c-93f9-4da8684d75f6


# ╔═╡ 6a7724fb-7ec1-4247-af95-70306b7f432a
test_par_df

# ╔═╡ bb79dcc1-84a1-4674-8a3e-9e3296ee542d


# ╔═╡ 25d833e7-b699-4627-9da9-0c8b862623d0


# ╔═╡ deb67934-24ef-442f-9ac0-528b440ef251


# ╔═╡ 4a1dc108-7077-42da-9b40-c48e08daf2e5


# ╔═╡ d42e8555-9ecf-419c-9c5a-63e3ac5838a0


# ╔═╡ 3763c237-14e8-4a40-b4c7-091ba0bdb8f9
typeof(test_par_df)

# ╔═╡ 580ff48b-486e-4c7c-a2cc-20fa690b1675


# ╔═╡ 3efe9777-e10b-432b-956c-a0ab69d7a952
ppc[1][1]

# ╔═╡ 9565d25e-afe1-4b3f-adec-52d4065c9c99


# ╔═╡ 29b023e3-879d-438e-bef8-f290bf3d4a55
begin
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit_bm.pdf")))
...
begin
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit.pdf")))
		savefig(p_ppc, projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit.pdf"))
		savefig(p_ppc, projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit.png"))
		savefig(p_ppc, projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit.svg"))
	end
	
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit_bm.pdf")))
		savefig(p_ppc_bm, projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit_bm.pdf"))
		savefig(p_ppc_bm, projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit_bm.png"))
		savefig(p_ppc_bm, projectdir("notebooks", "02_fitting", notebook_folder,"ppc_fit_bm.svg"))
	end
end
...
begin
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"df_mcmc_comp.jlso")))
		...
		JLSO.save(projectdir("notebooks", "02_fitting", notebook_folder,"df_mcmc_comp.jlso"), :df_par_all => df_par_all)
		save(projectdir("notebooks", "02_fitting", notebook_folder,"df_mcmc_comp.csv"), df_par_all)
	else
		df_par_all = JLSO.load(projectdir("notebooks", "02_fitting", notebook_folder,"df_mcmc_comp.jlso"))[:df_par_all]
...
if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"logp_3d_mat.jlso")))
...
begin
	if !(isfile(projectdir("notebooks", "02_fitting", notebook_folder,"logp_3d_mat.jlso")))
		...
		for j in 1:size(loglike_3d,3)
			DelimitedFiles.writedlm(projectdir("notebooks", "02_fitting", notebook_folder,"logp_3d_mat_$(j).txt"), loglike_3d[:,:,j])
		end
		
		JLSO.save(projectdir("notebooks", "02_fitting", notebook_folder,"logp_3d_mat.jlso"), :loglike_3d=>loglike_3d)
	else
		loglike_3d = JLSO.load(projectdir("notebooks", "02_fitting", notebook_folder,"logp_3d_mat.jlso"))[:loglike_3d]
	end
	loglike_3d
end
...
# ╔═╡ ca01af44-a417-4642-973d-f1359a7ee03c
svg_joinpathsplit__FILE__1assetsfavicon_unsaturatedsvg = let
    import PlutoUI
    PlutoUI.LocalResource(joinpath(split(@__FILE__, '#')[1] * ".assets", "favicon_unsaturated.svg"))
end
