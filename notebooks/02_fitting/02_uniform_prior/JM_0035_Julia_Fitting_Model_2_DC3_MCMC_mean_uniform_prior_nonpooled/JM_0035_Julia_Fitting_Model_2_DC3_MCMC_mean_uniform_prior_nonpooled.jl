...
begin	
	include(projectdir("scripts", "run_project", "00_mcmc_settings_test.jl"))
...
p_init = ones([8,9,8,12][tryparse(Int, model_id)])
...
begin
	...
	for k in 1:length(donor_ids)
		for l in 1:2
			density!(p_diag_1, [rand(MyDistribution(priors.p_DC3bm, Uniform(0.0,2.0), Uniform(0.0,2.0),Uniform(0.0,2.0)))[l] for j in 1:1000], subplot=6*(k-1)+(l*2), c=:black, legend=true, label="prior")
		end
	end
