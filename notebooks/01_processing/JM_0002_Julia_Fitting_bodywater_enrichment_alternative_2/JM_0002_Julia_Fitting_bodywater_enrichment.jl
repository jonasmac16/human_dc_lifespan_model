...
folder_name = "JM_0002_Julia_Fitting_bodywater_enrichment_alternative_2"

# ╔═╡ 0aef9c7e-cae1-11ec-0ab1-2fe3cc83abe2
data_label = @pipe CSV.read(datadir("exp_pro", "glucose_data_revision.csv"), DataFrame) |>
subset(_, :individual => (x -> x .∈ Ref(["D01", "D02", "D04"])))# subset(_, AsTable([:individual, :time]) => (x -> .!((x.individual .∈ Ref(["D01", "D02", "D04"])) .& (x.time .== 1/3))))
...
begin
	Plots.plot(tp,  U_smooth_2stp.(tp, 1.09867293634531323,10.498700374705663, frac_exp, tau_stop, 0.25), lab="simulation")
	Plots.xlabel!("time (d)")
...
begin
	...
	for j in gdata_label 
		...
		tmodel = model_flat_prior(data_in, U_smooth_2stp, 4.0, 50.0, frac_exp, tau_stop)

		gprob_global = instantiate_galacticoptim_problem(tmodel, MLE(), constrained(), [0.0,0.0,0.0,eps(),eps()],[4.0,50.0,1.0,2.0,2.0])
		res_global_tmp = GalacticOptim.solve(gprob_global.prob, BBO(); maxiters=1e6);
		push!(res_mle_global, res_global_tmp)

		gprob_local = instantiate_galacticoptim_problem(tmodel, MLE(), constrained(), [0.0,0.0,0.0,eps(),eps()],[4.0,50.0,1.0,2.0,2.0]; init_vals =res_global_tmp.minimizer)
...
begin
	...
	for j in gdata_label 
		...
		gprob_global = instantiate_galacticoptim_problem(tmodel, MAP(), constrained(), [0.0,0.0,0.0,eps(),eps()],[4.0,50.0,1.0,2.0,2.0])
		res_global_tmp = GalacticOptim.solve(gprob_global.prob, BBO(); maxiters=1e6);
		push!(res_map_global, res_global_tmp)

		gprob_local = instantiate_galacticoptim_problem(tmodel, MAP(), constrained(), [0.0,0.0,0.0,eps(),eps()],[4.0,50.0,1.0,2.0,2.0]; init_vals =res_global_tmp.minimizer)
		res_local_tmp = GalacticOptim.solve(gprob_local.prob, Fminbox(LBFGS()); maxiters=1e6, allow_f_increases=false)
