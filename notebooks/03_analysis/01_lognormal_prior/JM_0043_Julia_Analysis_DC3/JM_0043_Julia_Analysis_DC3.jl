...
begin
	results_folders = @pipe [try j.captures[1] catch end for j in filter!(p -> p != nothing, match.(r"(JM_004[0-1]_.+)", readdir(projectdir("notebooks", "02_fitting","01_lognormal_prior"))))] |> _[[isfile(projectdir("notebooks", "02_fitting","01_lognormal_prior",j, "results", "logp_3d_mat.jlso")) for j in _]]
end
