...
begin
	results_folders = @pipe [try j.captures[1] catch end for j in filter!(p -> p != nothing, match.(r"(JM_004[3-5]_.+)", readdir(projectdir("notebooks", "02_fitting","01_lognormal_prior"))))] |> _[[isfile(projectdir("notebooks", "02_fitting","01_lognormal_prior",j, "results", "logp_3d_mat.jlso")) for j in _]]
end
...
begin
	...
	for l in 1:3
		...
		save(projectdir("notebooks", "03_analysis","01_lognormal_prior", notebook_folder, "Parameter_posterior_summary_stats_DC3_model_"*string(l)*".csv"), _)

		@pipe df_par |>
		subset(_, :model_id => (x -> x .== string(l))) |>
		select(_, .![any(ismissing.(j)) for j in eachcol(_)]) |>
		save(projectdir("notebooks", "03_analysis","01_lognormal_prior", notebook_folder, "Parameter_full_posterior_DC3_model_"*string(l)*".csv"), _)
	end
end
...
begin
	...
	gcf()
	PyPlot.savefig(projectdir("notebooks", "03_analysis","01_lognormal_prior", notebook_folder, "PSIS_LOO_CV_Model_comparison_DC3_leave_out_sample.pdf"))
	PyPlot.savefig(projectdir("notebooks", "03_analysis","01_lognormal_prior", notebook_folder, "PSIS_LOO_CV_Model_comparison_DC3_leave_out_sample.svg"))
end

# ╔═╡ fae12768-3fa0-46f3-8839-b91acbbceb99
begin
	## save model comparison df
	save(projectdir("notebooks", "03_analysis","01_lognormal_prior", notebook_folder, "PSIS_LOO_CV_Model_comparison_DC3_leave_out_sample.csv"), df_arviz_loo)
