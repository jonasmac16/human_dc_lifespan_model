function _model_5(du,u,p,t, U_func, R)
    ...
    p_ASDCbm, δ_ASDCbm, p_cDC1bm, δ_cDC1bm, p_cDC2bm, δ_cDC2bm, δ_ASDCb, δ_cDC1b, δ_cDC2b, λ_ASDC, λ_cDC1, λ_cDC2, Δ_cDC2bm, Δ_cDC2b = p[1:(end-4)]
    fr, delta, frac, tau = p[(end-3):end]

    U = U_func(t, fr, delta, frac, tau)

    du[1] = p_ASDCbm * U - (δ_ASDCbm + Δ_cDC2bm + λ_ASDC) * LASDCm ##ASDCm

    du[2] = p_cDC1bm * U - (δ_cDC1bm + λ_cDC1) * LcDC1m ##cDC1m

    du[3] = p_cDC2bm * U - (δ_cDC2bm + λ_cDC2) * LcDC2m + Δ_cDC2bm * R_precDC2bm * LASDCm ##cDC2m

    du[4] = λ_ASDC * R_ASDC * LASDCm - (δ_ASDCb + Δ_cDC2b) * LASDCb ##ASDCb
