function _model_5(du,u,p,t, U_func, R)
    ...
    du[1] = p_ASDCbm * U - (δ_ASDCbm + Δ_DC2bm + λ_ASDC) * LpreDCm ##preDCm

    du[2] = p_cDC1bm * U - (δ_cDC1bm + λ_cDC1) * LcDC1m ##cDC1m

    du[3] = p_DC2bm * U - (δ_DC2bm + λ_DC2) * LcDC2m + Δ_DC2bm * R_precDC2bm * LpreDCm ##cDC2m

    du[4] = λ_ASDC * R_preDC * LpreDCm - (δ_ASDCb + Δ_DC2b) * LpreDCb ##preDCb

    du[5] = λ_cDC1 * R_cDC1 * LcDC1m - δ_cDC1b*LcDC1b ##cDC1b

    du[6] = λ_DC2 * R_cDC2 * LcDC2m + Δ_DC2b * R_precDC2b * LpreDCb - δ_DC2b*LcDC2b ##cDC2b
end