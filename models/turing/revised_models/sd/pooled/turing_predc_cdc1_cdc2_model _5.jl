...
function Base.rand(d::MyDistribution)
    ...
    b[8] = rand(truncated(d.dΔ_cDC2b,0.0, -5e-12 + (b[1] - b[7])*d.RASDC)) #Δ_cDC2b
    b[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8]/d.RASDC)*d.RASDC)) #δ_ASDCb
    
    
    upper_λ_cDC1 = b[2]
    upper_λ_cDC2 = b[3] + b[7] * d.RASDC_cDC2_bm
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[8] = rand(truncated(d.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*d.RASDC)) #Δ_cDC2b
    x[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8]/d.RASDC)*d.RASDC)) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_cDC2 = x[3] + x[7] * d.RASDC_cDC2_bm
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_cDC2b,0.0, -5e-12 + (b[1] - b[7])*d.RASDC), b[8]) #Δ_cDC2b
    l += logpdf(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8]/d.RASDC)*d.RASDC), b[4]) #δ_ASDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[8] = bijector(truncated(b.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*b.RASDC))(x[8]) #Δ_cDC2b
    y[4] = bijector(truncated(b.dδ_ASDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8]/b.RASDC)*b.RASDC))(x[4]) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_cDC2 = x[3] + x[7] * b.RASDC_cDC2_bm
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[8] = inv(bijector(truncated(b.orig.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*b.orig.RASDC)))(y[8]) #Δ_cDC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8]/b.orig.RASDC)*b.orig.RASDC)))(y[4]) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_cDC2 = x[3] + x[7] * b.orig.RASDC_cDC2_bm
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*b.RASDC)),x[8]) #Δ_cDC2b
    l += logabsdetjac(bijector(truncated(b.dδ_ASDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8]/b.RASDC)*b.RASDC)),x[4]) #δ_ASDCb
...
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_ASDCbm,d.dp_cDC1bm,d.dp_cDC2bm,d.dδ_ASDCb,d.dΔ_cDC2bm,d.dΔ_cDC2b,d.dλ_cDC1,d.dλ_cDC2,d.RASDC,d.RASDC_cDC1_bm,d.RASDC_cDC2_bm)
...
@model function _turing_model(data::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, sim::Bool=false, logp::Bool=false; ode_args = (;))
    ...
    p_ASDCbm, p_cDC1bm, p_cDC2bm, δ_ASDCb, λ_cDC1, λ_cDC2, Δ_cDC2bm, Δ_cDC2b = par           
    λ_ASDC = (Δ_cDC2b + δ_ASDCb) / data.R.RASDC
    
    σ ~ filldist(InverseGamma(2,3),3)


    ### compound parameter
    δ_ASDCbm = p_ASDCbm .- λ_ASDC .-  Δ_cDC2bm
    δ_cDC1bm = p_cDC1bm .- λ_cDC1
    δ_cDC2bm = p_cDC2bm .+ Δ_cDC2bm .* data.R.RASDC_cDC2_bm .- λ_cDC2
    δ_cDC1b = λ_cDC1 .* data.R.RcDC1
    δ_cDC2b = λ_cDC2 .* data.R.RcDC2 .+ Δ_cDC2b .* data.R.RASDC_cDC2_blood

    theta = [[p_ASDCbm, δ_ASDCbm, p_cDC1bm, δ_cDC1bm, p_cDC2bm, δ_cDC2bm, δ_ASDCb, δ_cDC1b, δ_cDC2b, λ_ASDC, λ_cDC1, λ_cDC2, Δ_cDC2bm, Δ_cDC2b] for j in 1:metadata.n_indv]
    ...
    parameters =(;p_ASDCbm=p_ASDCbm, δ_ASDCbm=δ_ASDCbm, p_cDC1bm=p_cDC1bm, δ_cDC1bm=δ_cDC1bm, p_cDC2bm=p_cDC2bm, δ_cDC2bm=δ_cDC2bm, δ_ASDCb=δ_ASDCb, δ_cDC1b=δ_cDC1b, δ_cDC2b=δ_cDC2b, λ_ASDC=λ_ASDC, λ_cDC1=λ_cDC1, λ_cDC2=λ_cDC2, Δ_cDC2bm=Δ_cDC2bm, Δ_cDC2b=Δ_cDC2b))
end
