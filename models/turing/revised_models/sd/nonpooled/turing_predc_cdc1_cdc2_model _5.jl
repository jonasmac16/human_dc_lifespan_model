...
function Base.rand(d::MyDistribution)
    ...
    b[8] = rand(truncated(d.dΔ_DC2b,0.0, -5e-12 + (b[1] - b[7])*d.RASDC)) #Δ_DC2b
    b[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8]/d.RASDC)*d.RASDC)) #δ_ASDCb
    
    
    upper_λ_cDC1 = b[2]
    upper_λ_DC2 = b[3] + b[7] * d.RASDC_DC2_bm
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[8] = rand(truncated(d.dΔ_DC2b,0.0, -5e-12 + (x[1] - x[7])*d.RASDC)) #Δ_DC2b
    x[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8]/d.RASDC)*d.RASDC)) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_DC2 = x[3] + x[7] * d.RASDC_DC2_bm
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_DC2b,0.0, -5e-12 + (b[1] - b[7])*d.RASDC), b[8]) #Δ_DC2b
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[8] = bijector(truncated(b.dΔ_DC2b,0.0, -5e-12 + (x[1] - x[7])*b.RASDC))(x[8]) #Δ_DC2b
    y[4] = bijector(truncated(b.dδ_ASDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8]/b.RASDC)*b.RASDC))(x[4]) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_DC2 = x[3] + x[7] * b.RASDC_DC2_bm
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[8] = inv(bijector(truncated(b.orig.dΔ_DC2b,0.0, -5e-12 + (x[1] - x[7])*b.orig.RASDC)))(y[8]) #Δ_DC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8]/b.orig.RASDC)*b.orig.RASDC)))(y[4]) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_DC2 = x[3] + x[7] * b.orig.RASDC_DC2_bm
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_DC2b,0.0, -5e-12 + (x[1] - x[7])*b.RASDC)),x[8]) #Δ_DC2b
...
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_ASDCbm,d.dp_cDC1bm,d.dp_DC2bm,d.dδ_ASDCb,d.dΔ_DC2bm,d.dΔ_DC2b,d.dλ_cDC1,d.dλ_DC2,d.RASDC,d.RASDC_cDC1_bm,d.RASDC_DC2_bm)
...
@model function _turing_model(data::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, sim::Bool=false, logp::Bool=false; ode_args = (;))
    ...
    p_ASDCbm, p_cDC1bm, p_DC2bm, δ_ASDCb, λ_cDC1, λ_DC2, Δ_DC2bm, Δ_DC2b = eachrow(par)           
    λ_ASDC = (Δ_DC2b .+ δ_ASDCb) ./ data.R.RASDC
    ...
    δ_DC2b = λ_DC2 .* data.R.RDC2 .+ Δ_DC2b .* data.R.RASDC_DC2_blood
