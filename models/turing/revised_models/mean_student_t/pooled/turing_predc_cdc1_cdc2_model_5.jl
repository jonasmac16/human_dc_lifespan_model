...
function Base.rand(d::MyDistribution)
    ...
    b[8] = rand(truncated(d.dΔ_cDC2b,0.0, -5e-12 + (b[1] - b[7])*d.R_ASDC)) #Δ_cDC2b
    b[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8]/d.R_ASDC)*d.R_ASDC)) #δ_ASDCb
    
    
    upper_λ_cDC1 = b[2]
    upper_λ_cDC2 = b[3] + b[7] * d.R_precDC2bm
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[8] = rand(truncated(d.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*d.R_ASDC)) #Δ_cDC2b
    x[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8]/d.R_ASDC)*d.R_ASDC)) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_cDC2 = x[3] + x[7] * d.R_precDC2bm
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_cDC2b,0.0, -5e-12 + (b[1] - b[7])*d.R_ASDC), b[8]) #Δ_cDC2b
    l += logpdf(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8]/d.R_ASDC)*d.R_ASDC), b[4]) #δ_ASDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[8] = bijector(truncated(b.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*b.R_ASDC))(x[8]) #Δ_cDC2b
    y[4] = bijector(truncated(b.dδ_ASDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8]/b.R_ASDC)*b.R_ASDC))(x[4]) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_cDC2 = x[3] + x[7] * b.R_precDC2bm
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[8] = inv(bijector(truncated(b.orig.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*b.orig.R_ASDC)))(y[8]) #Δ_cDC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8]/b.orig.R_ASDC)*b.orig.R_ASDC)))(y[4]) #δ_ASDCb
    
    
    upper_λ_cDC1 = x[2]
    upper_λ_cDC2 = x[3] + x[7] * b.orig.R_precDC2bm
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC2b,0.0, -5e-12 + (x[1] - x[7])*b.R_ASDC)),x[8]) #Δ_cDC2b
    l += logabsdetjac(bijector(truncated(b.dδ_ASDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8]/b.R_ASDC)*b.R_ASDC)),x[4]) #δ_ASDCb
...
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_ASDCbm,d.dp_cDC1bm,d.dp_cDC2bm,d.dδ_ASDCb,d.dΔ_cDC2bm,d.dΔ_cDC2b,d.dλ_cDC1,d.dλ_cDC2,d.R_ASDC,d.R_precDC1bm,d.R_precDC2bm)
...
@model function _turing_model(data::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple; ode_parallel_mode=EnsembleSerial(), ode_args = (;))
    ...
    ### priors
    par ~ MyDistribution(priors.p_ASDCbm, priors.p_cDC1bm, priors.p_cDC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0),R_ASDC, R_precDC1bm,R_precDC2bm)
    p_ASDCbm, p_cDC1bm, p_cDC2bm, δ_ASDCb, λ_cDC1, λ_cDC2, Δ_cDC2bm, Δ_cDC2b = par           
    λ_ASDC = (Δ_cDC2b + δ_ASDCb) / R_ASDC
    
    σ ~ filldist(TruncatedNormal(0.0, 1.0, 0.0,Inf),3)
    ν ~ filldist(LogNormal(2.0, 1.0),3)
    ...
    data ~ arraydist(LocationScale.(map(j -> sol[metadata.order.donor[j]][metadata.order.population[j], metadata.order.timepoint_idx[j]], 1:length(metadata.order.donor)), σ[metadata.order.population], TDist.(ν[metadata.order.population])))

   

    ## generated_quantities
    return (;sol =sol,
    log_likelihood = logpdf.(LocationScale.(map(j -> sol[metadata.order.donor[j]][metadata.order.population[j], metadata.order.timepoint_idx[j]], 1:length(metadata.order.donor)), σ[metadata.order.population], TDist.(ν[metadata.order.population])), data),
