...
function Base.rand(d::MyDistribution)
    ...
    b[8] = rand(truncated(d.dΔ_DC2b,0.0, -3e-12 + (b[1] - b[5] -b[6]-b[7]/d.R_preDC)*d.R_preDC)) #Δ_DC2b
    b[4] = rand(truncated(d.dδ_ASDCb,0.0, -2e-12 +  (b[1] - b[5] -b[6]-b[7]/d.R_preDC-b[8]/d.R_preDC)*d.R_preDC)) #δ_ASDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[8] = rand(truncated(d.dΔ_DC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/d.R_preDC)*d.R_preDC)) #Δ_DC2b
    x[4] = rand(truncated(d.dδ_ASDCb,0.0, -2e-12 +  (x[1] - x[5] -x[6]-x[7]/d.R_preDC-x[8]/d.R_preDC)*d.R_preDC)) #δ_ASDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_DC2b,0.0, -3e-12 + (b[1] - b[5] -b[6]-b[7]/d.R_preDC)*d.R_preDC), b[8]) #Δ_DC2b
    l += logpdf(truncated(d.dδ_ASDCb,0.0, -2e-12 +  (b[1] - b[5] -b[6]-b[7]/d.R_preDC-b[8]/d.R_preDC)*d.R_preDC), b[4]) #δ_ASDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[8] = bijector(truncated(b.dΔ_DC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.R_preDC)*b.R_preDC))(x[8]) #Δ_DC2b
    y[4] = bijector(truncated(b.dδ_ASDCb,0.0,  -2e-12 + (x[1] - x[5] -x[6]-x[7]/b.R_preDC-x[8]/b.R_preDC)*b.R_preDC))(x[4]) #δ_ASDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[8] = inv(bijector(truncated(b.orig.dΔ_DC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.orig.R_preDC)*b.orig.R_preDC)))(y[8]) #Δ_DC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_ASDCb,0.0, -2e-12 +  (x[1] - x[5] -x[6]-x[7]/b.orig.R_preDC-x[8]/b.orig.R_preDC)*b.orig.R_preDC)))(y[4]) #δ_ASDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_DC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.R_preDC)*b.R_preDC)),x[8]) #Δ_DC2b
    l += logabsdetjac(bijector(truncated(b.dδ_ASDCb,0.0,  -2e-12 + (x[1] - x[5] -x[6]-x[7]/b.R_preDC-x[8]/b.R_preDC)*b.R_preDC)),x[4]) #δ_ASDCb

    return l
end
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_ASDCbm,d.dp_cDC1bm,d.dp_DC2bm,d.dδ_ASDCb,d.dΔ_cDC1bm,d.dΔ_DC2bm,d.dΔ_cDC1b,d.dΔ_DC2b,d.R_preDC,d.R_precDC1bm,d.R_precDC2bm)
...
@model function _turing_model(data::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple; ode_parallel_mode=EnsembleSerial(), ode_args = (;))
    ...
    δ_DC2bm = p_DC2bm .+ Δ_DC2bm .* R_precDC2bm
    δ_cDC1b = Δ_cDC1b .* R_precDC1b
    δ_DC2b = Δ_DC2b .* R_precDC2b
