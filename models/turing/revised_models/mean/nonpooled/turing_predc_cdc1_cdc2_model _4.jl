...
function Base.rand(d::MyDistribution)
    ...
    b[7] = rand(truncated(d.dΔ_cDC1b,0.0, -4e-12 + (b[1] - b[5] -b[6])*d.RpreDC)) #Δ_cDC1b
    b[8] = rand(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (b[1] - b[5] -b[6]-b[7]/d.RpreDC)*d.RpreDC)) #Δ_cDC2b
    b[4] = rand(truncated(d.dδ_preDCb,0.0, -2e-12 +  (b[1] - b[5] -b[6]-b[7]/d.RpreDC-b[8]/d.RpreDC)*d.RpreDC)) #δ_preDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[7] = rand(truncated(d.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*d.RpreDC)) #Δ_cDC1b
    x[8] = rand(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/d.RpreDC)*d.RpreDC)) #Δ_cDC2b
    x[4] = rand(truncated(d.dδ_preDCb,0.0, -2e-12 +  (x[1] - x[5] -x[6]-x[7]/d.RpreDC-x[8]/d.RpreDC)*d.RpreDC)) #δ_preDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_cDC1b,0.0, -4e-12 + (b[1] - b[5] -b[6])*d.RpreDC), b[7]) #Δ_cDC1b
    l += logpdf(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (b[1] - b[5] -b[6]-b[7]/d.RpreDC)*d.RpreDC), b[8]) #Δ_cDC2b
    l += logpdf(truncated(d.dδ_preDCb,0.0, -2e-12 +  (b[1] - b[5] -b[6]-b[7]/d.RpreDC-b[8]/d.RpreDC)*d.RpreDC), b[4]) #δ_preDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[7] = bijector(truncated(b.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*b.RpreDC))(x[7]) #Δ_cDC1b
    y[8] = bijector(truncated(b.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.RpreDC)*b.RpreDC))(x[8]) #Δ_cDC2b
    y[4] = bijector(truncated(b.dδ_preDCb,0.0,  -2e-12 + (x[1] - x[5] -x[6]-x[7]/b.RpreDC-x[8]/b.RpreDC)*b.RpreDC))(x[4]) #δ_preDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[7] = inv(bijector(truncated(b.orig.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*b.orig.RpreDC)))(y[7]) #Δ_cDC1b
    x[8] = inv(bijector(truncated(b.orig.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.orig.RpreDC)*b.orig.RpreDC)))(y[8]) #Δ_cDC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_preDCb,0.0, -2e-12 +  (x[1] - x[5] -x[6]-x[7]/b.orig.RpreDC-x[8]/b.orig.RpreDC)*b.orig.RpreDC)))(y[4]) #δ_preDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*b.RpreDC)),x[7]) #Δ_cDC1b
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.RpreDC)*b.RpreDC)),x[8]) #Δ_cDC2b
    l += logabsdetjac(bijector(truncated(b.dδ_preDCb,0.0,  -2e-12 + (x[1] - x[5] -x[6]-x[7]/b.RpreDC-x[8]/b.RpreDC)*b.RpreDC)),x[4]) #δ_preDCb

    return l
end
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_preDCbm,d.dp_cDC1bm,d.dp_cDC2bm,d.dδ_preDCb,d.dΔ_cDC1bm,d.dΔ_cDC2bm,d.dΔ_cDC1b,d.dΔ_cDC2b,d.RpreDC,d.RpreDC_cDC1_bm,d.RpreDC_cDC2_bm)
...
@model function _turing_model(data::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, sim::Bool=false, logp::Bool=false; ode_args = (;))
    ...
    p_preDCbm, p_cDC1bm, p_cDC2bm, δ_preDCb, Δ_cDC1bm, Δ_cDC2bm, Δ_cDC1b, Δ_cDC2b = eachrow(par)           
    λ_preDC = (Δ_cDC1b .+ Δ_cDC2b .+ δ_preDCb) ./ data.R.RpreDC
    ...
    δ_cDC1bm = p_cDC1bm .+ Δ_cDC1bm .* data.R.RpreDC_cDC1_bm
    δ_cDC2bm = p_cDC2bm .+ Δ_cDC2bm .* data.R.RpreDC_cDC2_bm
    δ_cDC1b = Δ_cDC1b .* data.R.RpreDC_cDC1_blood
    δ_cDC2b = Δ_cDC2b .* data.R.RpreDC_cDC2_blood

    theta = [[p_preDCbm[j], δ_preDCbm[j], p_cDC1bm[j], δ_cDC1bm[j], p_cDC2bm[j], δ_cDC2bm[j], δ_preDCb[j], δ_cDC1b[j], δ_cDC2b[j], λ_preDC[j], Δ_cDC1bm[j], Δ_cDC2bm[j], Δ_cDC1b[j], Δ_cDC2b[j]] for j in 1:metadata.n_indv]
