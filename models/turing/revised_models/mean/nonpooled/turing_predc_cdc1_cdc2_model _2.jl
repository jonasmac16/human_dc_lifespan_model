...
function Base.rand(d::MyDistribution)
    ...
    b[4] = rand(truncated(d.dδ_preDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8])*d.RpreDC)) #δ_preDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[4] = rand(truncated(d.dδ_preDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8])*d.RpreDC)) #δ_preDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dδ_preDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8])*d.RpreDC), b[4]) #δ_preDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[4] = bijector(truncated(b.dδ_preDCb,0.0,  -2e-12 + (x[1] - x[7] -x[8])*b.RpreDC))(x[4]) #δ_preDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[4] = inv(bijector(truncated(b.orig.dδ_preDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8])*b.orig.RpreDC)))(y[4]) #δ_preDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dδ_preDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8])*b.RpreDC)),x[4]) #δ_preDCb
...
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_preDCbm,d.dp_cDC1bm,d.dp_cDC2bm,d.dδ_preDCb,d.dΔ_cDC1bm,d.dΔ_cDC2bm,d.dλ_cDC1,d.dλ_cDC2,d.RpreDC,d.RpreDC_cDC1_bm,d.RpreDC_cDC2_bm)
...
@model function _turing_model(data::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, sim::Bool=false, logp::Bool=false; ode_args = (;))
    ...
    p_preDCbm, p_cDC1bm, p_cDC2bm, δ_preDCb, λ_cDC1, λ_cDC2, Δ_cDC1bm, Δ_cDC2bm = eachrow(par)           
    ...
    δ_cDC1bm = p_cDC1bm .+ Δ_cDC1bm .* data.R.RpreDC_cDC1_bm .- λ_cDC1
    δ_cDC2bm = p_cDC2bm .+ Δ_cDC2bm .* data.R.RpreDC_cDC2_bm .- λ_cDC2
    δ_cDC1b = λ_cDC1 .* data.R.RcDC1
    δ_cDC2b = λ_cDC2 .* data.R.RcDC2

    theta = [[p_preDCbm[j], δ_preDCbm[j], p_cDC1bm[j], δ_cDC1bm[j], p_cDC2bm[j], δ_cDC2bm[j], δ_preDCb[j], δ_cDC1b[j], δ_cDC2b[j], λ_preDC[j], λ_cDC1[j], λ_cDC2[j], Δ_cDC1bm[j], Δ_cDC2bm[j]] for j in 1:metadata.n_indv]
