...
@model function _turing_model(data::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, ::Type{T} = Float64; ode_parallel_mode=EnsembleSerial(), ode_args = (;)) where {T}
    ...
    #     p_ASDCbm[j], p_cDC1bm[j], p_DC2bm[j], δ_ASDCb[j], λ_cDC1[j], λ_DC2[j], Δ_cDC1bm[j], Δ_DC2bm[j], Δ_cDC1b[j], Δ_DC2b[j] ~ MvNormal(zeros(10), ones(10))#MyDistribution(priors.p_ASDCbm, priors.p_cDC1bm, priors.p_DC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), R_ASDC, R_precDC1bm, R_preDC2bm)
    # end
    
    prior_dist = MyDistribution(priors.p_ASDCbm, priors.p_cDC1bm, priors.p_DC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), R_ASDC, R_precDC1bm, R_preDC2bm)
