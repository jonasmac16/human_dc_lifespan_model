...
@model function _turing_model(data::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, ::Type{T} = Float64; ode_parallel_mode=EnsembleSerial(), ode_args = (;)) where {T}
    ...
    ### priors
    # p_ASDCbm, p_cDC1bm, p_cDC2bm, δ_ASDCb, λ_cDC1, λ_cDC2, Δ_cDC1bm, Δ_cDC2bm, Δ_cDC1b, Δ_cDC2b = (Vector{T}(undef, metadata.n_indv) for _ = 1:10)
    # for j in 1:metadata.n_indv
    #     p_ASDCbm[j], p_cDC1bm[j], p_cDC2bm[j], δ_ASDCb[j], λ_cDC1[j], λ_cDC2[j], Δ_cDC1bm[j], Δ_cDC2bm[j], Δ_cDC1b[j], Δ_cDC2b[j] ~ MvNormal(zeros(10), ones(10))#MyDistribution(priors.p_ASDCbm, priors.p_cDC1bm, priors.p_cDC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), R_ASDC, R_precDC1bm, R_precDC2bm)
    # end
