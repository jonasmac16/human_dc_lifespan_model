...
function Base.rand(d::MyDistribution)
    ...
    b[10] = rand(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (b[1] - b[7] -b[8]-b[9]/d.RpreDC)*d.RpreDC)) #Δ_cDC2b
    b[4] = rand(truncated(d.dδ_preDCb,0.0, -2e-12 +  (b[1] - b[7] -b[8]-b[9]/d.RpreDC-b[10]/d.RpreDC)*d.RpreDC)) #δ_preDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[10] = rand(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[7] -x[8]-x[9]/d.RpreDC)*d.RpreDC)) #Δ_cDC2b
    x[4] = rand(truncated(d.dδ_preDCb,0.0, -2e-12 +  (x[1] - x[7] -x[8]-x[9]/d.RpreDC-x[10]/d.RpreDC)*d.RpreDC)) #δ_preDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (b[1] - b[7] -b[8]-b[9]/d.RpreDC)*d.RpreDC), b[10]) #Δ_cDC2b
    l += logpdf(truncated(d.dδ_preDCb,0.0, -2e-12 +  (b[1] - b[7] -b[8]-b[9]/d.RpreDC-b[10]/d.RpreDC)*d.RpreDC), b[4]) #δ_preDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[10] = bijector(truncated(b.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[7] -x[8]-x[9]/b.RpreDC)*b.RpreDC))(x[10]) #Δ_cDC2b
    y[4] = bijector(truncated(b.dδ_preDCb,0.0,  -2e-12 + (x[1] - x[7] -x[8]-x[9]/b.RpreDC-x[10]/b.RpreDC)*b.RpreDC))(x[4]) #δ_preDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[10] = inv(bijector(truncated(b.orig.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[7] -x[8]-x[9]/b.orig.RpreDC)*b.orig.RpreDC)))(y[10]) #Δ_cDC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_preDCb,0.0, -2e-12 +  (x[1] - x[7] -x[8]-x[9]/b.orig.RpreDC-x[10]/b.orig.RpreDC)*b.orig.RpreDC)))(y[4]) #δ_preDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[7] -x[8]-x[9]/b.RpreDC)*b.RpreDC)),x[10]) #Δ_cDC2b
    l += logabsdetjac(bijector(truncated(b.dδ_preDCb,0.0,  -2e-12 + (x[1] - x[7] -x[8]-x[9]/b.RpreDC-x[10]/b.RpreDC)*b.RpreDC)),x[4]) #δ_preDCb
...
@model function _turing_model(data::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, ode_parallel_mode=EnsembleSerial(), priors::NamedTuple; ode_args = (;))
	### priors
    par ~ MyDistribution(priors.p_preDCbm, priors.p_cDC1bm, priors.p_cDC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0),metadata.R.RpreDC, metadata.R.RpreDC_cDC1_bm,metadata.R.RpreDC_cDC2_bm)
    p_preDCbm, p_cDC1bm, p_cDC2bm, δ_preDCb, λ_cDC1, λ_cDC2, Δ_cDC1bm, Δ_cDC2bm, Δ_cDC1b, Δ_cDC2b = par
    λ_preDC = (Δ_cDC1b + Δ_cDC2b + δ_preDCb) / metadata.R.RpreDC
    
    σ ~ filldist(InverseGamma(2,3),3)

    ### compound parameter
    δ_preDCbm = p_preDCbm .- λ_preDC .- Δ_cDC1bm .- Δ_cDC2bm
    δ_cDC1bm = p_cDC1bm .+ Δ_cDC1bm .* metadata.R.RpreDC_cDC1_bm .- λ_cDC1
    δ_cDC2bm = p_cDC2bm .+ Δ_cDC2bm .* metadata.R.RpreDC_cDC2_bm .- λ_cDC2
    δ_cDC1b = λ_cDC1 .* metadata.R.RcDC1 .+ Δ_cDC1b .* metadata.R.RpreDC_cDC1_blood
    δ_cDC2b = λ_cDC2 .* metadata.R.RcDC2 .+ Δ_cDC2b .* metadata.R.RpreDC_cDC2_blood
