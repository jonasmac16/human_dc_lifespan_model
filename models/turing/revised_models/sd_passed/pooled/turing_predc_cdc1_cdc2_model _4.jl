...
function Base.rand(d::MyDistribution)
    ...
    b[7] = rand(truncated(d.dΔ_cDC1b,0.0, -4e-12 + (b[1] - b[5] -b[6])*d.RASDC)) #Δ_cDC1b
    b[8] = rand(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (b[1] - b[5] -b[6]-b[7]/d.RASDC)*d.RASDC)) #Δ_cDC2b
    b[4] = rand(truncated(d.dδ_ASDCb,0.0, -2e-12 +  (b[1] - b[5] -b[6]-b[7]/d.RASDC-b[8]/d.RASDC)*d.RASDC)) #δ_ASDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[7] = rand(truncated(d.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*d.RASDC)) #Δ_cDC1b
    x[8] = rand(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/d.RASDC)*d.RASDC)) #Δ_cDC2b
    x[4] = rand(truncated(d.dδ_ASDCb,0.0, -2e-12 +  (x[1] - x[5] -x[6]-x[7]/d.RASDC-x[8]/d.RASDC)*d.RASDC)) #δ_ASDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dΔ_cDC1b,0.0, -4e-12 + (b[1] - b[5] -b[6])*d.RASDC), b[7]) #Δ_cDC1b
    l += logpdf(truncated(d.dΔ_cDC2b,0.0, -3e-12 + (b[1] - b[5] -b[6]-b[7]/d.RASDC)*d.RASDC), b[8]) #Δ_cDC2b
    l += logpdf(truncated(d.dδ_ASDCb,0.0, -2e-12 +  (b[1] - b[5] -b[6]-b[7]/d.RASDC-b[8]/d.RASDC)*d.RASDC), b[4]) #δ_ASDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[7] = bijector(truncated(b.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*b.RASDC))(x[7]) #Δ_cDC1b
    y[8] = bijector(truncated(b.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.RASDC)*b.RASDC))(x[8]) #Δ_cDC2b
    y[4] = bijector(truncated(b.dδ_ASDCb,0.0,  -2e-12 + (x[1] - x[5] -x[6]-x[7]/b.RASDC-x[8]/b.RASDC)*b.RASDC))(x[4]) #δ_ASDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[7] = inv(bijector(truncated(b.orig.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*b.orig.RASDC)))(y[7]) #Δ_cDC1b
    x[8] = inv(bijector(truncated(b.orig.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.orig.RASDC)*b.orig.RASDC)))(y[8]) #Δ_cDC2b
    x[4] = inv(bijector(truncated(b.orig.dδ_ASDCb,0.0, -2e-12 +  (x[1] - x[5] -x[6]-x[7]/b.orig.RASDC-x[8]/b.orig.RASDC)*b.orig.RASDC)))(y[4]) #δ_ASDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC1b,0.0, -4e-12 + (x[1] - x[5] -x[6])*b.RASDC)),x[7]) #Δ_cDC1b
    l += logabsdetjac(bijector(truncated(b.dΔ_cDC2b,0.0, -3e-12 + (x[1] - x[5] -x[6]-x[7]/b.RASDC)*b.RASDC)),x[8]) #Δ_cDC2b
    l += logabsdetjac(bijector(truncated(b.dδ_ASDCb,0.0,  -2e-12 + (x[1] - x[5] -x[6]-x[7]/b.RASDC-x[8]/b.RASDC)*b.RASDC)),x[4]) #δ_ASDCb

    return l
end
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_ASDCbm,d.dp_cDC1bm,d.dp_cDC2bm,d.dδ_ASDCb,d.dΔ_cDC1bm,d.dΔ_cDC2bm,d.dΔ_cDC1b,d.dΔ_cDC2b,d.RASDC,d.RASDC_cDC1_bm,d.RASDC_cDC2_bm)
...
@model function _turing_model(data::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, sim::Bool=false, logp::Bool=false; ode_args = (;))
	### priors
    par ~ MyDistribution(priors.p_ASDCbm, priors.p_cDC1bm, priors.p_cDC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0),data.R.RASDC, data.R.RASDC_cDC1_bm,data.R.RASDC_cDC2_bm)
    p_ASDCbm, p_cDC1bm, p_cDC2bm, δ_ASDCb, Δ_cDC1bm, Δ_cDC2bm, Δ_cDC1b, Δ_cDC2b = par           
    λ_ASDC = (Δ_cDC1b + Δ_cDC2b + δ_ASDCb) / data.R.RASDC
    ...
    δ_cDC1bm = p_cDC1bm .+ Δ_cDC1bm .* data.R.RASDC_cDC1_bm
    δ_cDC2bm = p_cDC2bm .+ Δ_cDC2bm .* data.R.RASDC_cDC2_bm
    δ_cDC1b = Δ_cDC1b .* data.R.RASDC_cDC1_blood
    δ_cDC2b = Δ_cDC2b .* data.R.RASDC_cDC2_blood

    theta = [[p_ASDCbm, δ_ASDCbm, p_cDC1bm, δ_cDC1bm, p_cDC2bm, δ_cDC2bm, δ_ASDCb, δ_cDC1b, δ_cDC2b, λ_ASDC, Δ_cDC1bm, Δ_cDC2bm, Δ_cDC1b, Δ_cDC2b] for j in 1:metadata.n_indv]
    ...
    ## hierachical error
    ### sd passed
    μ ~ MvNormal(sol[metadata.order.donor][metadata.order.population, metadata.order.timepoint_idx], σ[metadata.order.population])
    ...
    parameters =(;p_ASDCbm=p_ASDCbm, δ_ASDCbm=δ_ASDCbm, p_cDC1bm=p_cDC1bm, δ_cDC1bm=δ_cDC1bm, p_cDC2bm=p_cDC2bm, δ_cDC2bm=δ_cDC2bm, δ_ASDCb=δ_ASDCb, δ_cDC1b=δ_cDC1b, δ_cDC2b=δ_cDC2b, λ_ASDC=λ_ASDC, Δ_cDC1bm=Δ_cDC1bm, Δ_cDC2bm=Δ_cDC2bm, Δ_cDC1b=Δ_cDC1b, Δ_cDC2b=Δ_cDC2b))
end
