...
function Base.rand(d::MyDistribution)
    ...
    b[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8])*d.R_preDC)) #δ_ASDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[4] = rand(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8])*d.R_preDC)) #δ_ASDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dδ_ASDCb,0.0, -4e-12 +  (b[1] - b[7] -b[8])*d.R_preDC), b[4]) #δ_ASDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[4] = bijector(truncated(b.dδ_ASDCb,0.0,  -2e-12 + (x[1] - x[7] -x[8])*b.R_preDC))(x[4]) #δ_ASDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[4] = inv(bijector(truncated(b.orig.dδ_ASDCb,0.0, -4e-12 +  (x[1] - x[7] -x[8])*b.orig.R_preDC)))(y[4]) #δ_ASDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dδ_ASDCb,0.0,  -4e-12 + (x[1] - x[7] -x[8])*b.R_preDC)),x[4]) #δ_ASDCb
...
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_ASDCbm,d.dp_cDC1bm,d.dp_DC2bm,d.dδ_ASDCb,d.dΔ_cDC1bm,d.dΔ_DC2bm,d.dλ_cDC1,d.dλ_DC2,d.R_preDC,d.R_precDC1bm,d.R_precDC2bm)
...
@model function _turing_model(data::Array{Float64,1}, data_sd::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, ::Type{T} = Float64; ode_parallel_mode=EnsembleSerial(), ode_args = (;)) where {T}
    ...
    p_ASDCbm, p_cDC1bm, p_DC2bm, δ_ASDCb, λ_cDC1, λ_DC2, Δ_cDC1bm, Δ_DC2bm = assign_par(par, length(prior_dist), metadata.n_indv)          
    λ_ASDC = δ_ASDCb ./ R_preDC
    ...
    theta = [[p_ASDCbm[j], δ_ASDCbm[j], p_cDC1bm[j], δ_cDC1bm[j], p_DC2bm[j], δ_DC2bm[j], δ_ASDCb[j], δ_cDC1b[j], δ_DC2b[j], λ_ASDC[j], λ_cDC1[j], λ_DC2[j], Δ_cDC1bm[j], Δ_DC2bm[j]] for j in 1:metadata.n_indv]
