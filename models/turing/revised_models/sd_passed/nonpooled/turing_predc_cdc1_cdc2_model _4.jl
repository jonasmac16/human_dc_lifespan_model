...
@model function _turing_model(data::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, sim::Bool=false, logp::Bool=false; ode_args = (;))
    ...
    p_ASDCbm, p_cDC1bm, p_cDC2bm, δ_ASDCb, Δ_cDC1bm, Δ_cDC2bm, Δ_cDC1b, Δ_cDC2b = eachrow(par)           
    ...
    theta = [[p_ASDCbm[j], δ_ASDCbm[j], p_cDC1bm[j], δ_cDC1bm[j], p_cDC2bm[j], δ_cDC2bm[j], δ_ASDCb[j], δ_cDC1b[j], δ_cDC2b[j], λ_ASDC[j], Δ_cDC1bm[j], Δ_cDC2bm[j], Δ_cDC1b[j], Δ_cDC2b[j]] for j in 1:metadata.n_indv]
