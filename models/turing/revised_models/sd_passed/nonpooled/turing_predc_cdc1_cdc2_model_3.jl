...
function Base.rand(d::MyDistribution)
    ...
    b[4] = rand(truncated(d.dδ_preDCb,0.0, b[1]*d.R_preDC)) #δ_preDCb
...
function Distributions._rand!(rng::Random.AbstractRNG,d::MyDistribution, x::Array{Float64,1})
    ...
    x[4] = rand(truncated(d.dδ_preDCb,0.0, x[1]*d.R_preDC)) #δ_preDCb
...
function Distributions._logpdf(d::MyDistribution, b::AbstractVector)
    ...
    l += logpdf(truncated(d.dδ_preDCb,0.0, b[1]*d.R_preDC), b[4]) #δ_preDCb
...
function (b::MyBijector)(x::AbstractVector)
    ...
    y[4] = bijector(truncated(b.dδ_preDCb,0.0,  x[1]*b.R_preDC))(x[4]) #δ_preDCb
...
function (b::Inverse{<:MyBijector})(y::AbstractVector)
    ...
    x[4] = inv(bijector(truncated(b.orig.dδ_preDCb,0.0, x[1]*b.orig.R_preDC)))(y[4]) #δ_preDCb
...
function Bijectors.logabsdetjac(b::MyBijector, x::AbstractVector)
    ...
    l += logabsdetjac(bijector(truncated(b.dδ_preDCb,0.0,  x[1]*b.R_preDC)),x[4]) #δ_preDCb
...
Bijectors.bijector(d::MyDistribution)= MyBijector(d.dp_preDCbm,d.dp_cDC1bm,d.dp_cDC2bm,d.dδ_preDCb,d.dλ_cDC1,d.dλ_cDC2,d.R_preDC,d.R_precDC1bm,d.R_precDC2bm)
...
@model function _turing_model(data::Array{Float64,1}, data_sd::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, ::Type{T} = Float64; ode_parallel_mode=EnsembleSerial(), ode_args = (;)) where {T}
    ...
    ### priors
    prior_dist = MyDistribution(priors.p_preDCbm, priors.p_cDC1bm, priors.p_cDC2bm, Uniform(0.0,2.0), Uniform(0.0,2.0), Uniform(0.0,2.0),R_preDC, R_precDC1bm,R_precDC2bm)
    par = Vector{Array{T,1}}(undef, metadata.n_indv)
    for j in 1:metadata.n_indv
        par[j] ~ prior_dist
    end
        
    p_preDCbm, p_cDC1bm, p_cDC2bm, δ_preDCb, λ_cDC1, λ_cDC2 = assign_par(par, length(prior_dist), metadata.n_indv)          
    ...
    ### compound parameter
    δ_preDCbm = p_preDCbm .- λ_preDC
    δ_cDC1bm = p_cDC1bm .- λ_cDC1
    δ_cDC2bm = p_cDC2bm .- λ_cDC2
    δ_cDC1b = λ_cDC1 .* R_cDC1
    δ_cDC2b = λ_cDC2 .* R_cDC2

    theta = [[p_preDCbm[j], δ_preDCbm[j], p_cDC1bm[j], δ_cDC1bm[j], p_cDC2bm[j], δ_cDC2bm[j], δ_preDCb[j], δ_cDC1b[j], δ_cDC2b[j], λ_preDC[j], λ_cDC1[j], λ_cDC2[j]] for j in 1:metadata.n_indv]
    ...
    parameters =(;p_preDCbm=p_preDCbm, δ_preDCbm=δ_preDCbm, p_cDC1bm=p_cDC1bm, δ_cDC1bm=δ_cDC1bm, p_cDC2bm=p_cDC2bm, δ_cDC2bm=δ_cDC2bm, δ_preDCb=δ_preDCb, δ_cDC1b=δ_cDC1b, δ_cDC2b=δ_cDC2b, λ_preDC=λ_preDC, λ_cDC1=λ_cDC1, λ_cDC2=λ_cDC2))
end
