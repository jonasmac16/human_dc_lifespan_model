...
@model function _turing_model(data::Array{Float64,1}, data_sd::Array{Float64,1}, metadata::NamedTuple, ode_prob::ODEProblem, solver, priors::NamedTuple, ::Type{T} = Float64; ode_parallel_mode=EnsembleSerial(), ode_args = (;)) where {T}
    ...
    p_ASDCbm, p_cDC1bm, p_cDC2bm, δ_ASDCb, λ_cDC1, λ_cDC2, Δ_cDC1bm, Δ_cDC2bm = assign_par(par, length(prior_dist), metadata.n_indv)          
    ...
    theta = [[p_ASDCbm[j], δ_ASDCbm[j], p_cDC1bm[j], δ_cDC1bm[j], p_cDC2bm[j], δ_cDC2bm[j], δ_ASDCb[j], δ_cDC1b[j], δ_cDC2b[j], λ_ASDC[j], λ_cDC1[j], λ_cDC2[j], Δ_cDC1bm[j], Δ_cDC2bm[j]] for j in 1:metadata.n_indv]
